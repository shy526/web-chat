var svg = d3.select('.bliblicolor').append('svg')
    .attr('width', 320)
    .attr('height', 600);
var x1 = 160,
    y1 = 150;
...
function saveColor(color) {
    getKey("colors",function (data) {
        ...
        if(!data||!Array.isArray(data)){
            //如果为空就设置默认值
            data=["#2ae0c8", "#a2e1d4", "#fe6673", "#fbb8ac", "#fad8be", "#e3c887", "#e6e2c3", "#bdf3d4", "#cbf5fb", "#acf6ef"];
        }else {
            data.push(color)
        }
        save({"colors": data})
...
$(function () {
    ...
    $("#colorsdefault").click(function () {
        var data=["#2ae0c8", "#a2e1d4", "#fe6673", "#fbb8ac", "#fad8be", "#e3c887", "#e6e2c3", "#bdf3d4", "#cbf5fb", "#acf6ef"];
        save({"colors": data})
        $(".colorblok").remove();
        for(var i=0;i<data.length;i++){
            addColor(data[i])
    ...
    $("#testbliblibli").click(function () {
        var str = $("#test").val();
        launch(str);
    })
    $("#saveblibliTime").click(function () {
        var time=$("#blibliTime").val().trim();
        if(!time){
            alert("弹幕过场时间不能为空")
        }else if(time>0){
            save({time:time})
...
function addColor(data) {
    var  html="<div class='colorblok' attr-data='"+data+"' style='background: "+data+"; '></div>"
    //追加颜色
    $(".bliblix").append(html);
    /**
     * 添加鼠标移动事件
...
function colorblokEvent(){
    //取消绑定
    $(".colorblok").unbind()
    $(".colorblok").mouseover(function () {
        var s_color=$(this).attr("attr-data");
        setColor('#show_circle', s_color);
        d3.select('#color_code').text(s_color).attr('fill', s_color).attr('stroke', s_color);
    })
    //添加一个删除事件
    $(".colorblok").click(function () {
        var s_color=$(this).attr("attr-data");
        getKey("colors",function (data) {
                var arr=new Array()
            for(var i=0;i<data.length;i++){
